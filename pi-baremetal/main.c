...
void testmmu(unsigned int *UFCON0){
        const char *p="helloworld MMU\n";
        while(*p){
                *UFCON0=*p++;
        };
}
...
void main(unsigned int r0, unsigned int machtype, unsigned int atagsaddr)
{
	...
	mem_init();
        ...
        unsigned int VCADDR = mem_p2v(0x20201000);
static volatile unsigned int *gpioGPSET0 = (unsigned int *) mem_p2v(0x20201000);
        testmmu(gpioGPSET0);
        testmmu(0xA0203000);
	led_init();
	fb_init();
	...
	console_write(tohex((unsigned int)&_kend, 4));
         ...
         console_write(FG_YELLOW"\n\nUART MMU ADDRESS:     0x");
        console_write(tohex((unsigned int)VCADDR,4));
